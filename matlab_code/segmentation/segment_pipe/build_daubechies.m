function recon_im = build_daubechies(daub_im,display)
% 'build_daubechies' reverses the actions of 'daubechies_97', in that it
% recomposes the image from the scaling images (at the lowest resolution in
% daub_im) and the difference images (vertical, horizontal, and diagonal)
% at each scale. 
...
% set 'prev_im' to be the lowest resolution scaling image in 'daub_im'
...
for i=dp:-1:1
    ...
    im_LL = imfilter(im_LL,kern_l','circular'); % lowpass filter vertically

    im_LH = kron(daub_im{i,2},[0;1]); % upsample vertically
    im_LH = imfilter(im_LH,kern_h','circular'); % highpass filter vertically
    
    im_L = im_LL + im_LH; % combine the scaling and vertical detail images
    im_L = kron(im_L,[1 0]); % upsample horizontally
    im_L = imfilter(im_L,kern_l,'circular'); % lowpass filter horizontally
    
    im_HL = kron(daub_im{i,3},[1;0]); % upsample vertically
    im_HL = imfilter(im_HL,kern_l','circular'); % lowpass filter vertically

    im_HH = kron(daub_im{i,4},[0;1]); % upsample vertically
    im_HH = imfilter(im_HH,kern_h','circular'); % highpass filter vertically
    
    im_H = im_HL + im_HH; % combine the horizontal and diagonal detail images
    im_H = kron(im_H,[0 1]); % upsample horizontally
    im_H = imfilter(im_H,kern_h,'circular'); % highpass filter horizontally

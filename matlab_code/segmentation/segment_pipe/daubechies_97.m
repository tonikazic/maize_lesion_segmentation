...
for i=1:depth
    ...
    im_L = im_L(:,reshape(vertcat(2+(0:(wd/4-1))*4,3+(0:(wd/4-1))*4),wd/2,1)); % downsample horizontally
    
    im_H = imfilter(prev_LL,kern_h,'symmetric'); % highpass filter horizontally 
    im_H = im_H(:,reshape(vertcat(1+(0:(wd/4-1))*4,4+(0:(wd/4-1))*4),wd/2,1)); % downsample horizontally 
    
    im_LL = imfilter(im_L,kern_l','symmetric'); % lowpass filter vertically
    im_LL = im_LL(reshape(vertcat(2+(0:(ht/4-1))*4,3+(0:(ht/4-1))*4),ht/2,1),:); % downsample vertically
    
    im_LH = imfilter(im_L,kern_h','symmetric'); % highpass filter vertically
    im_LH = im_LH(reshape(vertcat(1+(0:(ht/4-1))*4,4+(0:(ht/4-1))*4),ht/2,1),:); % downsample vertically
    
    im_HL = imfilter(im_H,kern_l','symmetric'); % lowpass filter vertically
    im_HL = im_HL(reshape(vertcat(2+(0:(ht/4-1))*4,3+(0:(ht/4-1))*4),ht/2,1),:); % downsample vertically
    
    im_HH = imfilter(im_H,kern_h','symmetric'); % highpass filter vertically
    im_HH = im_HH(reshape(vertcat(1+(0:(ht/4-1))*4,4+(0:(ht/4-1))*4),ht/2,1),:); % downsample vertically
